...
class BayesianBlocksSignificanceTest:
    ...
    def _extract_light_curve_values(self, lc_df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        Extract flux, errors, and time values from light curve DataFrame.
        ...
        """
        # Check if it's a dict (from older pickle format)
        if isinstance(lc_df, dict):
            lc_df = pd.DataFrame.from_dict(lc_df, orient='index')
        
        # Extract flux values - handle various formats
        if 'flux' in lc_df.columns:
            flux_col = lc_df['flux']
        elif 'flux_b' in lc_df.columns:
            flux_col = lc_df['flux_b']
        else:
        ...
        # Extract errors - handle various formats
        if 'errors' in lc_df.columns:
            errors = lc_df['errors'].values
        else:
            # Try to find error columns
            error_cols = [col for col in lc_df.columns if 'error' in col.lower()]
            if len(error_cols) >= 2:
                ...
                # Default: use 10% of flux as error
                error_lower = flux_array * 0.1
                error_upper = flux_array * 0.1
                return flux_array, error_lower, error_upper, lc_df['t'].values, lc_df['tw'].values
        ...
        if 't' in lc_df.columns:
            times = lc_df['t'].values
        else:
            times = np.arange(len(flux_array))
        
        if 'tw' in lc_df.columns:
            time_widths = lc_df['tw'].values
        else:
            time_widths = np.ones(len(flux_array))
    ...
    def load_source_data(self, source_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if self.verbose:
                ...
                print(f"Number of Bayesian Blocks: {len(self.light_curve_df)}\n")
    ...
    def generate_bootstrap_sample(self, iteration: int) -> BootstrapResult:
        ...
        result = BootstrapResult(
            ...
            block_edges=np.array([0, len(bootstrap_flux)]),  # Simplified for now
    ...
    def count_blocks_in_bootstrap(self, bootstrap_flux: np.ndarray) -> int:
        ...
        A more sophisticated approach would use actual Bayesian Blocks algorithm.
    ...
    def run_significance_test(self) -> List[BootstrapResult]:
        ...
        for i in range(self.num_bootstrap_iterations):
            try:
                ...
                # (In practice, you'd use actual Bayesian Blocks algorithm here)
                ...
                pbar.set_description(f"Bootstrap: {i+1}/{self.num_bootstrap_iterations} "
                                   f"(avg blocks: {np.mean([r.num_blocks for r in self.bootstrap_results]):.1f})")
    ...
    def plot_significance_results(self, figsize: Tuple[float, float] = (14, 10)) -> plt.Figure:
        ...
        original = self.original_structure
        bootstrap_blocks = np.array([r.num_blocks for r in self.bootstrap_results])
        bootstrap_fluxes = np.array([np.mean(r.flux_values) for r in self.bootstrap_results])
