"""
Random AGN Source Analysis from agnsteps Database
Performs comprehensive analysis of a randomly selected source from source_info_v1.pkl
...
class AGNStepAnalyzer:
    ...
    def __init__(self, source_name: str = None, 
        ...
        """
        Initialize analyzer for a specific source or random source.
        
        Args:
            source_name: Name of source to analyze. If None, selects a random source.
        ...
        self.is_random = source_name is None
    ...
    def _select_random_source_with_lightcurve(self, vdb: dict) -> str:
        ...
        Returns:
            Name of randomly selected source
        """
        # Filter sources that have light curves
        sources_with_lc = [
        ...
        selected = random.choice(sources_with_lc)
        return selected
        
    def _normalize_light_curve(self, lc) -> pd.DataFrame:
        """
        Normalize the light curve representation into a standard DataFrame
        with one row per Bayesian block and named columns.
        """
    ...
    def load_source_data(self) -> bool:
        ...
        try:
            ...
            if self.is_random:
                actual_name = self._select_random_source_with_lightcurve(vdb)
                print(f"{'='*80}")
                print(f"RANDOM SOURCE SELECTION")
                print(f"{'='*80}")
                print(f"✓ Randomly selected source: {actual_name}\n")
            else:
            ...
            # Normalize light curve representation into a standard DataFrame
            lc = source_dict['light_curve']
            self.light_curve_df = self._normalize_light_curve(lc)
    ...
    def analyze_light_curve_structure(self) -> Dict:
        ...
        error_lower = np.array([abs(e[0]) if isinstance(e, (list, tuple)) else abs(e) for e in errors], dtype=float)
        error_upper = np.array([abs(e[1]) if isinstance(e, (list, tuple)) else abs(e) for e in errors], dtype=float)
    ...
    def calculate_flux_ratios(self, margin_weeks: int = 50) -> Dict:
        ...
        if lc is None or len(lc) < 2:
            return {
                'type': 'single_block',
                'num_blocks': len(lc) if lc is not None else 0
            }
    ...
    def print_detailed_analysis(self, margin_weeks: int = 50) -> None:
        ...
        if ratios.get('type') == 'single_block':
            print("  Type: Single Block Source")
            print("  No transition ratio analysis available for a single-block light curve.")
        elif ratios['type'] == 'single_step':
    ...
    def plot_ratio_analysis(self, figsize: Tuple[float, float] = (10, 6)) -> plt.Figure:
        ...
        if ratios.get('type') == 'single_block':
            ...
            ax2.set_xticks([])
            ax2.set_yticks([])
            ax2.set_title('No Transition Ratios', fontsize=12, fontweight='bold')
        elif ratios['type'] == 'single_step':
...
"""
        ...
        if ratios.get('type') == 'single_block':
            report += "  • No transition ratios available for a single-block source.\n"
...
if __name__ == "__main__":
    ...
    # Create analyzer for a RANDOM source from the database
    # Pass None (or omit source_name) to select a random source
    analyzer = AGNStepAnalyzer(
        source_name=None,  # None = random selection!
    ...
    if analyzer.load_source_data():
        ...
        print(f"✓ Saved light curve plot: {source_safe_name}_light_curve.png")
        
        # Ratio analysis plot
        fig2 = analyzer.plot_ratio_analysis()
        plt.savefig(f'{source_safe_name}_ratios.png', dpi=150, bbox_inches='tight')
        print(f"✓ Saved ratio analysis plot: {source_safe_name}_ratios.png")
        
        plt.show()
        
    else:
        print("Failed to load source data. Check file path.")